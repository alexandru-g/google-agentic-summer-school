...
you met yesterday morning — it hedges or *invents* answers about our fictional
...
1. In `tools.py`, change `list_games`'s docstring to just `"Does stuff."`,
   ...
   `Which game has the best ratings?` — watch it fumble or skip the tool.
...
don't raise. *(Why not raise? Tomorrow, Part 2, we break tools on purpose.)*
...
being told the steps. Yesterday YOU were the orchestrator. Look at you now,
...
> filters results to one game (mask `df` *and* the vectors — Day 1, exercise 5.1),
...
- Ask the same question twice in two fresh sessions — same tool path? *(Remember
  temperature. Nondeterminism is Friday's whole topic.)*

Patch the instruction for whatever leaked. This loop — probe, patch, re-probe —
...
**Tomorrow:** session state (real memory) and a retrieval tool over the docs
